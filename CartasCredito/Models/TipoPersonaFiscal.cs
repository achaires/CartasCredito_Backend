...
namespace CartasCredito.Models
{
	public class TipoPersonaFiscal
	{
		...
		public static List<TipoPersonaFiscal> Get(int activo = 1)
		{
			List<TipoPersonaFiscal> res = new List<TipoPersonaFiscal>();
			...
			try
			{
				...
				var errores = "";
				if (da.Cons_TiposPersonaFiscal(out dt, out errores, activo))
				{
					if (dt.Rows.Count > 0)
					{
						for (int i = 0; i < dt.Rows.Count; i++)
						{
							int idx = 0;
							var row = dt.Rows[i];
							var item = new TipoPersonaFiscal();
			...
			{
				Console.Write(ex);
				res = new List<TipoPersonaFiscal>();
		...
		public static TipoPersonaFiscal GetById(int id)
		{
			TipoPersonaFiscal rsp = new TipoPersonaFiscal();
			...
			try
			{
				...
				if (da.Cons_TipoPersonaFiscalById(id, out dt, out errores))
				{
					if (dt.Rows.Count > 0)
					{
						var idx = 0;
			...
			catch (Exception ex)
			{
				...
				rsp = new TipoPersonaFiscal();
			}

			return rsp;
		}

		public static RespuestaFormato Insert(TipoPersonaFiscal modelo)
		{
			...
			try
			{
				...
				if (da.Ins_TipoPersonaFiscal(modelo, out dt, out errores))
				{
					if (dt.Rows.Count > 0)
					{
						var row = dt.Rows[0];
		...
		public static RespuestaFormato Update(TipoPersonaFiscal modelo)
		{
			RespuestaFormato rsp = new RespuestaFormato();
			...
			try
			{
				...
				if (da.Upd_TipoPersonaFiscal(modelo, out dt, out errores))
				{
					if (dt.Rows.Count > 0)
					{
						var row = dt.Rows[0];
