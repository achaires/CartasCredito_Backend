...
namespace CartasCredito.Models
{
	public class AgenteAduanal
	{
		...
		public static List<AgenteAduanal> Get(int activo = 1)
		{
			List<AgenteAduanal> res = new List<AgenteAduanal>();
			...
			try
			{
				...
				var errores = "";
				if (da.Cons_AgentesAduanales(out dt, out errores, activo))
				{
					if (dt.Rows.Count > 0)
					{
		...
		public static AgenteAduanal GetById(int id)
		{
			AgenteAduanal rsp = new AgenteAduanal();
			...
			try
			{
				...
				if (da.Cons_AgenteAduanalById(id, out dt, out errores))
				{
					if (dt.Rows.Count > 0)
					{
						var idx = 0;
			...
			catch (Exception ex)
			{
				...
				rsp = new AgenteAduanal();
			}

			return rsp;
		}

		public static RespuestaFormato Insert(AgenteAduanal modelo)
		{
			...
			try
			{
				...
				if (da.Ins_AgenteAduanal(modelo, out dt, out errores))
				{
					if (dt.Rows.Count > 0)
					{
						var row = dt.Rows[0];
		...
		public static RespuestaFormato Update(AgenteAduanal modelo)
		{
			RespuestaFormato rsp = new RespuestaFormato();
			...
			try
			{
				...
				if (da.Upd_AgenteAduanal(modelo, out dt, out errores))
				{
					if (dt.Rows.Count > 0)
					{
						var row = dt.Rows[0];
