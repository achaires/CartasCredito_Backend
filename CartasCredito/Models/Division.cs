...
namespace CartasCredito.Models
{
	public class Division
	{
		...
		public static List<Division> Get(int activo = 1)
		{
			List<Division> res = new List<Division>();
			...
			try
			{
				...
				var errores = "";
				if (da.Cons_Divisiones(out dt, out errores, activo))
				{
					if (dt.Rows.Count > 0)
					{
		...
		public static Division GetById(int id)
		{
			...
			try
			{
				...
				if (da.Cons_DivisionById(id, out dt, out errores))
				{
					if (dt.Rows.Count > 0)
					{
						var idx = 0;
		...
		public static RespuestaFormato Insert(Division modelo)
		{
			...
			try
			{
				...
				if (da.Ins_Division(modelo, out dt, out errores))
				{
					if (dt.Rows.Count > 0)
					{
						var row = dt.Rows[0];
		...
		public static RespuestaFormato Update(Division modelo)
		{
			RespuestaFormato rsp = new RespuestaFormato();
			...
			try
			{
				...
				if (da.Upd_Division(modelo, out dt, out errores))
				{
					if (dt.Rows.Count > 0)
					{
						var row = dt.Rows[0];
